...
function applyFilters(){updateFilterBtn();let src=getSource();if(filters.acts.length)src=src.filter(c=>filters.acts.includes(c.act));if(filters.themes.length)src=src.filter(c=>c.themes.some(t=>filters.themes.includes(t)));if(filters.techniques.length)src=src.filter(c=>c.devices.some(d=>filters.techniques.includes(d)));cards.deck=[...src];sortDeck();cards.index=0;cards.isFlipped=false;buildFilterChips();render()}
...
function rebuildAll(){while(app.allQuotes.length>app.builtInCount)app.allQuotes.pop();app.customQuotes.forEach(c=>app.allQuotes.push(c));app.bestQuotes=app.currentText.bestIdx.filter(i=>i<app.allQuotes.length).map(i=>app.allQuotes[i]);cards.deck=[...getSource()];sortDeck();cards.index=0;cards.isFlipped=false;buildFilterChips()}
...
// ══════ URL ROUTING ══════
function initRouter(){window.addEventListener('hashchange',handleRoute);handleRoute()}function handleRoute(){const hash=window.location.hash.slice(1);if(!hash||hash==='home'){navGo('home');return}const parts=hash.split('/');if(parts.length===1){openSubjectWithRoute(parts[0]);return}if(parts.length===2){openTextWithRoute(parts[0],parts[1]);return}if(parts.length===3){const toolView=document.getElementById('tool-view');if(app.currentSubject===parts[0]&&app.currentTextId===parts[1]&&toolView&&toolView.classList.contains('active'))return;openToolWithRoute(parts[0],parts[1],parts[2]);return}if(parts.length===4){const toolView=document.getElementById('tool-view');if(app.currentSubject===parts[0]&&app.currentTextId===parts[1]&&toolView&&toolView.classList.contains('active'))return;openToolWithRoute(parts[0],parts[1],parts[2],parts[3])}}function openSubjectWithRoute(id){window.location.hash=id;openSubject(id)}function openTextWithRoute(subjId,textId){window.location.hash=`${subjId}/${textId}`;openSubject(subjId);const waitForText=setInterval(()=>{const textCard=document.querySelector(`[data-text-id="${textId}"]`);if(textCard){clearInterval(waitForText);textCard.click()}},50)}function openToolWithRoute(subjId,textId,toolType,extra){const hash=extra?`${subjId}/${textId}/${toolType}/${extra}`:`${subjId}/${textId}/${toolType}`;window.location.hash=hash;openSubject(subjId);const waitForText=setInterval(()=>{const textCard=document.querySelector(`[data-text-id="${textId}"]`);if(textCard){clearInterval(waitForText);textCard.click();const waitForTool=setInterval(()=>{const toolBtn=document.querySelector(`[data-tool="${toolType}"]`);if(toolBtn){clearInterval(waitForTool);toolBtn.click()}},50)}},50)}
