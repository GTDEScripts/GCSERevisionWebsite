...
// ══════ URL ROUTING ══════
function initRouter(){window.addEventListener('hashchange',handleRoute);handleRoute()}function handleRoute(){const hash=window.location.hash.slice(1);if(!hash||hash==='home'){navGo('home');return}const parts=hash.split('/');if(parts.length===1){openSubjectWithRoute(parts[0]);return}if(parts.length===2){openTextWithRoute(parts[0],parts[1]);return}if(parts.length===3){openToolWithRoute(parts[0],parts[1],parts[2]);return}if(parts.length===4){openToolWithRoute(parts[0],parts[1],parts[2],parts[3])}}function openSubjectWithRoute(id){window.location.hash=id;openSubject(id)}function openTextWithRoute(subjId,textId){window.location.hash=`${subjId}/${textId}`;openSubject(subjId);setTimeout(()=>{const textCard=document.querySelector(`[data-text-id="${textId}"]`);if(textCard)textCard.click()},100)}function openToolWithRoute(subjId,textId,toolType,extra){const hash=extra?`${subjId}/${textId}/${toolType}/${extra}`:`${subjId}/${textId}/${toolType}`;window.location.hash=hash;openSubject(subjId);setTimeout(()=>{const textCard=document.querySelector(`[data-text-id="${textId}"]`);if(textCard)textCard.click();setTimeout(()=>{const toolBtn=document.querySelector(`[data-tool="${toolType}"]`);if(toolBtn)toolBtn.click()},100)},100)}
