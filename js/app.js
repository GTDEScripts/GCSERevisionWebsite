...
function backToParent(){if(app.parentSubject){window.location.hash=app.parentSubject;openSubject(app.parentSubject)}else{backToHome()}}
function backToSubject(){window.location.hash=app.currentSubject;document.getElementById('tool-view').classList.remove('active');document.getElementById('subject-view').classList.add('active');if(exam.timer){clearInterval(exam.timer);exam.timer=null}window.scrollTo(0,0)}
